...
import { query,create,getbyID,remove,update,deleteone} from 'services/bronk_personage_api'
...
export default modelExtend(pageModel, {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === '/database/personage') {
  ...
  effects: {
    * aliupload ({ payload = {} }, { call, put, select }) {
      const {clientOss,aliExpiration} = yield select(_ => _.bronk_personage)
   ...
   * delete ({ payload }, { call, put, select }) {
    const {contorlLimit,serchlist}  =   yield select(_ => _.bronk_personage)
      const data = yield call(remove,{id:payload})
 
      if (data.code===200) {
 
        yield put(routerRedux.push({
          pathname: '/database/personage',
          state:{
            serchlist,
            controlState:{
